...
class assign {
    ...
    protected function get_filtered_graders(int $userid, string $capability, bool $checkallocation = true): array {
        ...
        // If this course uses separate groups, get the list of groups for this student.
        // This list is needed whether we are searching for one grader or many.
        ...
        // MDL-46702: If marker allocation is active, check the allocated markers first.
        if ($checkallocation && $this->get_instance()->markingallocation) {
            $allocatedmarkers = $this->get_allocated_markers($userid);
            foreach ($allocatedmarkers as $allocatedmarker) {
                $allocatedmarkerid = $allocatedmarker->marker;
                if (is_enrolled($this->context, $allocatedmarkerid, $capability)) {
                    if ($this->verify_grader($userid, $allocatedmarkerid, $separategroups)) {
                        if ($user = core_user::get_user($allocatedmarkerid)) {
                            $filteredgraders[$allocatedmarkerid] = $user;
                        }
                    }
                }
            }
            // Finish early if possible.
            if (!empty($filteredgraders)) {
                return $filteredgraders;
            }
            // No valid markers were found, so we fall back to
            // checking all graders with the required capability.
        }

        // Check all users with the required capability in this context.
        $coursegraders = get_enrolled_users($this->context, $capability, 0, 'u.*');
        if ($coursegraders) {
            foreach ($coursegraders as $coursegrader) {
                if ($this->verify_grader($userid, $coursegrader->id, $separategroups)) {
                    $filteredgraders[$coursegrader->id] = $coursegrader;
                }
            }
        }
        return $filteredgraders;
